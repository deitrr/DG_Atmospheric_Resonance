...
#  cdo.daymean(input=str(output_merged),output=str(output_daymean))

#daily zonal mean
#output_dayzon = output_path / (args.sim_name[0] + '.cam.h1.zonmean_%04d_%04d.nc'%(start_year,end_year))

#if not output_dayzon.exists() or args.overwrite:
#  cdo.zonmean(input='-select,name=CLDHGH,CLDLOW,CLDMED,FSDS,IWC,LWC,PS,Q,T,TS,U,V,Z3 '+str(output_daymean),output=str(output_dayzon))
...
#  cdo.zonmean(input='-select,name=CLDHGH,CLDLOW,CLDMED,FSDS,IWC,LWC,PS,Q,TS,T,U,V,Z3 '+str(output_merged),output=str(output_zonmean))
...
#if not output_flut.exists() or args.overwrite:
#  cdo.select(input=',name=FLUT '+str(output_merged),output=str(output_flut))

#annual average
#output_annavg = output_path / (args.sim_name[0] + '.cam.h0.annavg_%04d_%04d.nc'%(1,end_year))
#if not output_annavg.exists() or args.overwrite:
#  cdo.yearmonmean(input='-select,name=T,TS,FLUT,FSNT,FLNT '+str(output_timeevol),output=str(output_annavg))
