...
#level = '1000'
#ilev = 39   #roughly 1000 hPa
#mean_lim = [-1.8e-6, 1.8e-6]
#mean_scale = [1e-6, '10$^{-6}$']
#anom_lim = [-8e-7, 8e-7]
#anom_scale = [1e-7, '10$^{-7}$']
#lm_lim = np.array([[-5e-7,5e-7],[-5e-7,5e-7]])
#lm_scale = [[1e-7, '10$^{-7}$'],[1e-7, '10$^{-7}$']]

#savelabel = field + level
...
#  p_anom_amp[i] = np.max(shmap)
  ...
  #ax = axes[i][0]
    ...
    c = m.pcolormesh(lon2d, lat2d, field_mean/mean_scale[0], cmap='PiYG',rasterized=True,latlon='True',vmax=mean_lim[ilevel][1]/mean_scale[0],vmin=mean_lim[ilevel][0]/mean_scale[0])
