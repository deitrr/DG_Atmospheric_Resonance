...
#            'solar0p9_lr_exocam_19x25_ch4-30_co2-5250_22hr_branch',
#            'solar0p9_lr_exocam_19x25_ch4-30_co2-5250_24hr_branch',
#            'solar0p9_lr_exocam_4x5_ch4-30_co2-50000_16hr_branch',
#            'solar0p9_lr_exocam_4x5_ch4-30_co2-50000_18hr_branch',
#            'solar0p9_lr_exocam_4x5_ch4-30_co2-50000_20hr_branch',
...
#            'solar0p9_lr_exocam_4x5_ch4-3000_co2-5250_20hr_branch',
...
rotpers = np.array([
#0.6667, 0.75, 0.8333,0.875, 0.8958, 0.9167, 0.9583, 1.0, 1.04167,
#                    0.6667, 0.75, 0.8333, 0.875, 0.8958, 0.9167, 0.9375, 0.9583, 1.0, 1.04167
#0.9271,
#0.875,  1.0
...
mean_lim = [0.03,0.35]
anom_lim = [-0.11, 0.11]
lm_lim = np.array([[-0.011,0.011],[-0.05,0.05]])
field = 'CWP'
clabel= 'Total Cloud WP\n(kg m$^{-2}$)'
savelabel = field
...
#    liwc = (data['LWC'][:]+data['IWC'][:])
    z3 = data['Z3'][:]
    lat = data['lat'][:]
    lon = data['lon'][:]

    field0 = data['TGCLDCWP'][:]
...
#  c = m.pcolormesh(lon2d, lat2d, field_mean/1000., cmap='plasma',rasterized=True,latlon='True',vmax=mean_lim[1],vmin=mean_lim[0])
...
#  m.drawparallels([-45,0,45],labels = [True,False,False,False], fontsize=6)
#  m.drawmeridians([-90,0,90],labels = [False,False,False,False], fontsize=6)
#  c = m.pcolormesh(lon2d, lat2d, field_anom_mean/1000.0, cmap=cmap,rasterized=True,latlon='True',vmax=anom_lim[1],vmin=anom_lim[0])
...
#    m.drawparallels([-45,0,45],labels = [True,False,False,False], fontsize=6)
#    m.drawmeridians([-90,0,90],labels = [False,False,False,False], fontsize=6)
#    c = m.pcolormesh(shlon2d, shlat2d, shmap_anom/1000.0, cmap=cmap,latlon='True',rasterized=True,vmax=lm_lim[imode-1][1],vmin=lm_lim[imode-1][0])
...
#plt.tight_layout()
#plt.savefig(parent_path + savelabel+'_dasl_s0p9_ch4-30_new.pdf')
#plt.close()
